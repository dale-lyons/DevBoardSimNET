...
; *************************************************************************
;
RegData        .text	" PC=  A=  X=  Y=  S=  P= (NVRBDIZC)="
;
...
;-----------DATA TABLES ------------------------------------------------
;
Hexdigdata     .text "0123456789ABCDEF";hex char table 
;     
...
OPCtxtData     .text "ADC"             ;0
               ...
               .text "WAI"             ;40
               .text "STP"             ;41
               .text "BBR"             ;42 4Byte Opcodes
               .text "BBS"             ;43
               .text "RMB"             ;44
               .text "SMB"             ;45
               .text ".DB"             ;46 define 1 byte for assembler
               .text ".DW"             ;47 define 1 word for assembler
               .text ".DS"             ;48 define a string block for assembler
               .text "???"             ;49 for invalid opcode
;
;
HelpTxt        .text "~Current commands are :~"
               .text "Syntax = {} required, [] optional, HHHH hex address, DD hex data~"
               .text "~"
               .text "[HHHH][ HHHH]{Return} - Hex dump address(s)(up to 16 if no address entered)~"
               .text "[HHHH]{.HHHH}{Return} - Hex dump range of addresses (16 per line)~"
               .text "[HHHH]{:DD}[ DD]{Return} - Change data bytes~"
               .text "[HHHH]{G}{Return} - Execute a program (use RTS to return to monitor)~"
               .text "{HHHH.HHHH>HHHH{I}{Return} - move range at 2nd HHHH down to 1st to 3rd HHHH~"
               .text "[HHHH]{L}{Return} - List (disassemble) 20 lines of program~"
               .text "[HHHH]{.HHHH}{L}{Return} - Dissassemble a range~"
               .text "{HHHH.HHHH>HHHH{M}{Return} - Move range at 1st HHHH thru 2nd to 3rd HHHH~"
               .text "[HHHH][ HHHH]{Q}{Return} - Text dump address(s)~"
               .text "[HHHH]{.HHHH}{Q}{Return} - Text dump range of addresses (16 per line)~"
               .text "{R}{Return} - Print register contents from memory locations~"
               .text "{U}{Return} - Upload File (Xmodem/CRC or Intel Hex)~"
               .text "{V}{Return} - Monitor Version~"
               .text "{HHHH.HHHH>HHHH{W}{Return} - Write data in RAM to EEPROM~"
               .text "{!}{Return} - Enter Assembler~"
               .text "{@}{Return} - Cold-Start Enhanced Basic~"
               .text "{#}{Return} - Warm_Start Enhanced Basic~"
               .text "{?}{Return} - Print menu of commands~~"
               .byte $00
AsmHelpTxt     .text "~Current commands are :~"
               .text "Syntax = {} required, [] optional~"
               .text "HHHH=hex address, OPC=Opcode, DD=hex data, '_'=Space Bar or Tab~"
               .text "'$' Symbols are optional, all values are HEX.~"
               .text "Any input after a 'semi-colon' is ignored.~"
               .text "~"
               .text "{HHHH}{Return} - Set input address~"
               .text "[HHHH][_]{OPC}[_][#($DD_HHHH,X),Y]{Return} - Assemble line~"
               .text "[HHHH]{L}{Return} - List (disassemble) 20 lines of program~"
               .text "{Return} - Exit Assembler back to Monitor~"
               .text "{?}{Return} - Print menu of commands~~"
               .byte $00
;
Porttxt        .text "65C02 Monitor v5.1 (5-30-05) Ready"
               .byte  $0d, $0a
               .text "with Enhanced Basic Interpreter (c) Lee Davison" 
               .byte  $0d, $0a
               .text "(Press ? for help)"
               .byte $00
;
