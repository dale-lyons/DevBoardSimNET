...
namespace ARM7
{
    ...
    /// <returns></returns>
    public delegate uint ReadMemoryDelegate(uint address, WordSize ms);

	/// <summary>
	/// This class provides some usefull utility routines.
	/// </summary>
	public static class Utils
	{
        ...
        public static string loadStringFromMemory(ReadMemoryDelegate readMemoryDelegate, uint address, uint maxSize)
        {
            ...
            try
            {
                ...
                do
                {
                    data = readMemoryDelegate(address++, WordSize.OneByte);
                    if (data == 0)
                        break;
            ...
            {
                System.Diagnostics.Debug.WriteLine("Error while reading string from memory:" + ex.Message);
                str.Length = 0;
            }//catch
            return str.ToString();
