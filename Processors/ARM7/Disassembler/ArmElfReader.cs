...
//
// Reads an object file in Elf format.
//
// Documentation for the format of entries in the relocation section
// comes from "ELF for the ARM® Architecture", GENC-003538 v1.04,
// 23rd January, 2007; Richard Earnshaw.
//     URL: http://www.arm.com/pdfs/aaelf.pdf
//
...
{
    public enum SHT
    {   // Section Header Table typecodes
    ...
    {  // Line number extended opcodes
        end_sequence = 1, set_address = 2, define_file = 3
    };
    ...
    public class ArmElfReader
    {
        ...
        protected int textStart, dataStart, bssStart;
        protected string compilationDirectory = null;
        ...
        {
            get { return compilationDirectory; }
        }

        public SourceLineReader SrcLineRdr { get { return srcLineRdr; } }
        ...
        public ArmElfReader(ArmFileInfo af, bool trace) : this(af)
        {
            // this.trace = trace? 1 : 0;
            this.trace = 2;
        }
        ...
        public void PerformPass1()
        {
            ...
            for (int i = 0; i < e_shnum; i++)
            {
                ...
                loadSectionTable((uint)i, SHT.STRTAB);
            }
            ...
            for (int i = 0; i < e_shnum; i++)
            {
                ...
                switch (typeCode)
                {
                    ...
                    case SHT.NOBITS:
                        ...
                        if (trace > 0)
                            Debug.WriteLine(String.Format(" {0} segment, size = {1}", s, size));
            ...
            for (int i = 0; i < e_shnum; i++)
            {
                ...
                loadSectionTable((uint)i, SHT.SYMTAB);
        ...
        public void PerformPass2(int[] startAddresses)
        {
            ...
            for (int i = 0; i < e_shnum; i++)
            {
                ...
                loadSectionTable((uint)i, SHT.PROGBITS);
                switch (name)
                {
                    case ".text":
            ...
            copyCodeSection(dataStart + roDataSize, dataSect, ".rodata.str1.4");
        ...
        protected void processSymbol(ST symbol)
        {
            ...
            //        if (globalSymbols.TryGetValue(symbol.name, out sym))
            //			throw new AsmException(
            //				"duplicate definition of global symbol {0}",
            //				symbol.name);
        ...
        protected string getString(ref int offset, byte[] section)
        {
            if (section == null)
        ...
        protected void loadSectionTable(uint whichSection, SHT whichKind)
        {
            byte[] sectionBytes = sectionTable[whichSection];
            if (sectionBytes != null) return;
            byte[] headerBytes = sectionHeaderTable[whichSection];
            uint type = getInt32(headerBytes, 4) & 0xff;
            if ((SHT)type != whichKind)
                throw new AsmException(
                    "section 0x{0:X} does not have typecode {1}",
                    whichSection, LookupType(whichKind));
        ...
        protected uint getInt32(byte[] arr, int offset)
        {
            uint result = 0;
            for (int i = 3; i >= 0; i--)
        ...
        {
            loadSectionTable(e_shstrndx, SHT.STRTAB);   // needed for getString() call
            for (int i = 0; i < e_shnum; i++)
            {
                byte[] htentry = sectionHeaderTable[i];
        ...
        protected void processSymTab(uint sectNum, byte[] htentry)
        {
            ...
            loadSectionTable(shIndexStr, SHT.STRTAB);
            ...
            while (size > 0)
            {
                ...
                if (len < 16)
                    throw new Exception("Read error (symtab entry");    // read error
        ...
        protected void processRelocationTable(byte[] htentry, bool abs)
        {
            ...
            if (trace > 0)
                Debug.WriteLine(String.Format(
                    ...
                    symtabNum, whichSect));
            loadSectionTable(whichSect, SHT.PROGBITS);
            ...
            while (size > 0)
            {
                ...
                if (len < relEntryLen)
                    throw new Exception("Read error (rel entry");   // read error
                ...
                performRelocation((uint)(sectStart + offset), offset, targetSection, addend, symbol, type);
            }
        }
                ...
                uint addr, int offset, byte[] section, uint addend, ST symbol, uint type)
        {
            ...
            if (name != null && name != "")
            {
                SyEntry sym = af.LookupSymbol(name);
                if (sym == null)
                {
        ...
        protected void processLineNumberInfo()
        {
            if (debugLineSect == null) return;
        ...
        protected void processDebugInfoEntry(ref int ix, List<int> abbreviationSectionTable)
        {
            ...
            DW_TAG tag = (DW_TAG)debugAbbrevSect[abix++];
            bool hasChildren = (debugAbbrevSect[abix++]) != 0;
                ...
                for (; ; )
                {
                    switch (f)
                    {
                        ...
                        case DW_FORM.ref_udata:
                            dummy = getLEB128(ref ix, debugInfoSect);
                            break;
                        case DW_FORM.strp:
        ...
        protected uint getLEB128(ref int ix, byte[] section)
        {
            uint result = 0;
            int shift = 0;
            for (; ; )
