...

{
    ...
    if (!q)
	{
		SYMBOL* a = (SYMBOL*)calloc(1, sizeof(SYMBOL) + strlen(nam));
		q = a;
		*p = a;

		if (!a)
		{
			fatal_error((char*)SYMBOLS);
			return (SYMBOL * )NULL;
		}
...
void lopen(const char *nam)
{
    if (list)
		warning((char*)TWOLST);
    else if (!(list = fopen(nam,"w")))
		fatal_error((char*)LSTOPEN);
    return;
}
...
void lputs(void)
{
	...
	do {
	    ...
	    check_page();
	    if (ferror(list)) fatal_error((char *)DSKFULL);
	} while (listhex && i);
    }
    return;
}
...
void lclose(void)
{
	...
	fprintf(list,"\f");
	if (ferror(list) || fclose(list) == EOF) fatal_error((char*)DSKFULL);
    }
    return;
}
...
void hopen(const char *nam)
{
    if (hex)
		warning((char*)TWOHEX);
    else if (!(hex = fopen(nam,"w")))
		fatal_error((char*)HEXOPEN);
    return;
}
...
void hclose(void)
{
    ...
    if (hex) {
	   if (cnt) record(0);
	   record(1);
	   if (fclose(hex) == EOF) fatal_error((char*)DSKFULL);
    }
    return;
}
...

{
    ...
    if (ferror(hex)) fatal_error((char*)DSKFULL);
    return;
}
